<?php
/**
 * Portofolio Content (untuk ditampilkan di dashboard.php)
 */
    ...
    if (empty($judul)) {
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                $new_filename = 'portofolio_' . time() . '.' . $ext;
        ...
        if (empty($error)) {
            ...
            $stmt->bind_param("ssss", $judul, $deskripsi, $foto, $tanggal_event);
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea 
                    ...
                    rows="3"
                    placeholder="Jelaskan detail portofolio ini"
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-all">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600 mt-1">Tanggal: <?php echo date('d F Y', strtotime($row['tanggal_event'] ?? 'now')); ?></p>
                                <p class="text-sm text-gray-600 mt-2"><?php echo htmlspecialchars(substr($row['deskripsi'], 0, 100)); ?>...</p>
                            </div>
                            <div class="ml-4 flex gap-2">
                                <a href="dashboard.php?page=portofolio&delete=<?php echo $row['id']; ?>" class="text-red-600 hover:text-red-700 text-sm font-medium" onclick="return confirm('Yakin ingin menghapus?')">
                                    Hapus
                                </a>
                            </div>
                        </div>
