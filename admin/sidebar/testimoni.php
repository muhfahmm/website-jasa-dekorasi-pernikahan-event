<?php
/**
 * Testimoni Content (untuk ditampilkan di dashboard.php)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    if (empty($nama_klien)) {
        ...
        if (empty($error)) {
            ...
            $stmt->bind_param("ssds", $nama_klien, $ulasan, $bintang, $foto_klien);
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <!-- Foto Klien -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Foto Klien</label>
                    <input 
                        type="file" 
                        name="foto_klien" 
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500"
                        accept="image/*"
                    >
                    <p class="text-xs text-gray-500 mt-1">JPG, PNG, atau GIF (Optional)</p>
                </div>
            </div>

            <!-- Ulasan -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Ulasan / Review</label>
                <textarea 
                    name="ulasan" 
                    ...
                    rows="3"
                    placeholder="Tuliskan ulasan dari klien"
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-all">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-yellow-600 mt-1">Rating: <?php echo str_repeat('⭐', $row['bintang']); ?></p>
                                <p class="text-sm text-gray-600 mt-2"><?php echo htmlspecialchars($row['ulasan']); ?></p>
                            </div>
                            <div class="ml-4 flex gap-2">
                                <a href="dashboard.php?page=testimoni&delete=<?php echo $row['id']; ?>" class="text-red-600 hover:text-red-700 text-sm font-medium" onclick="return confirm('Yakin ingin menghapus?')">
                                    Hapus
                                </a>
                            </div>
                        </div>
